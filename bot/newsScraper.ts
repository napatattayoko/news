...
function extractTickers(title: string): string[] {
  ...
  let tickers = match
    ? Array.from(new Set(match)).filter((word) => COMMON_TICKERS.has(word))
    : [];
...
function parseFinvizTime(
  timeStr: string,
  currentDateObj: { year: number; month: number; day: number },
) {
  ...
  if (parts.length >= 2) {
    if (parts[0].toLowerCase() === "today") {
      timePart = parts[1];
      const nyDateStr = new Date().toLocaleString("en-US", {
        timeZone: "America/New_York",
      });
      const nyDate = new Date(nyDateStr);
      newDateObj = {
        year: nyDate.getFullYear(),
        month: nyDate.getMonth(),
        day: nyDate.getDate(),
      };
    } else {
      datePart = parts[0];
  ...
  if (datePart && datePart.includes("-")) {
    ...
    if (datePieces.length === 2) {
      ...
      if (parsedMonth !== undefined && !isNaN(parsedDay)) {
        ...
        else if (currentDateObj.month === 11 && parsedMonth === 0)
          newDateObj.year++;
      }
    }
  }
  ...
  try {
    const calculatedDate = getUtcFromNewYork(
      newDateObj.year,
      newDateObj.month,
      newDateObj.day,
      hours,
...
export async function scrapeAndStoreNews() {
  ...
  try {
    ...
    const nyDateStr = new Date(
      nowUtc.getTime() + nyOffset * 3600 * 1000,
    ...
    $("table.styled-table-new tr").each((i, row) => {
      ...
      if (title && newsUrl) {
        const uniqueId = Buffer.from(title)
          .toString("base64")
          .replace(/\W/g, "")
          .substring(0, 30);
    ...
    console.log(
      `[NewsBot] Scraped ${rawNews.length} articles from Finviz. Checking DB for new ones...`,
    );
    ...
    console.log(
      `[NewsBot] Found ${newArticles.length} NEW articles. Need to categorize via AI...`,
    );
...
if (require.main === module) {
  ...
  const runContinuously = async () => {
    console.log(
      `[NewsBot] Starting continuous scraping mode (runs every 5 minutes)...`,
    );
    while (true) {
